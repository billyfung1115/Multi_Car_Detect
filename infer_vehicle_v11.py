...
WINDOW_NAME_IMAGE = "YOLOv11n - Image"
...
def load_model():
    ...
    print(f"Loading YOLOv11n model from: {weights_path}")
...
def infer_image(model, project_dir, source_path):
    ...
    out_dir = project_dir / "predictions_v11"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / f"{source_path.stem}_pred_v11.jpg"
...
def infer_video(model, project_dir, source_path):
    ...
    out_dir = project_dir / "predictions_v11"
    out_dir.mkdir(parents=True, exist_ok=True)
    out_path = out_dir / f"{source_path.stem}_pred_v11.mp4"
